...
Reports the image size, whether it already carries real transparency, the
sampled edge background color and its variation, and whether ``run.py`` would
route the image to the solid-color or the photo pipeline.
...
def main() -> None:
    """Run the diagnostics for the image path given on the command line."""
    if len(sys.argv) < 2:
